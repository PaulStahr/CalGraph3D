...
class MeshObject(OpticalObject):
    ...
    def getColor(
        ...
        if self.texture is not None:
            tCol = self.texture.getColor(textureCoordinate, xp)
    ...
    def getIntersection(self,
        ...
        if backend == "trimesh":
            ...
            # Create trimesh object once (cache if possible in your class!)
            if not hasattr(self, "_trimesh"):
                ...
                if not MeshObject._printed_trimesh_engine:
                    engine_module = self._trimesh.ray.__module__
                    pyembree = "pyembree" in engine_module
                    logger.log(logging.INFO, f"Using trimesh ray tracing engine for MeshObject (trimesh version: {trimesh.__version__}) with {'Embree backend (fast)' if pyembree else 'pure NumPy backend'}")

            # Perform ray intersection
            locations, index_ray, index_tri = self._trimesh.ray.intersects_location(
                ray_origins=ArrayUtil.convert(position, np),
                ray_directions=ArrayUtil.convert(direction, np),
                multiple_hits=False
            )
            ...
            locations = ArrayUtil.convert(locations, xp)
            index_tri = ArrayUtil.convert(index_tri, xp)
            index_ray = ArrayUtil.convert(index_ray, xp)
            ...
            # Compute distances in mesh space
            hit_vec = locations - position[index_ray]
            dist = xp.linalg.norm(hit_vec, axis=1)

            # Apply bounds filtering
            valid_mask = (
                    (dist >= lower_bound[index_ray]) &
                    (dist <= upper_bound[index_ray])
            )
            ...
            if self.smoothNormals:
                ...
                triangles = vertices[faces[index_tri]]
                bary = trimesh.triangles.points_to_barycentric(
                    triangles,
                    locations
                )

                vertex_normals = vertexNormals[faces[index_tri]]

                intersection.normal[index_ray] = (
                        bary[:, 0:1] * vertex_normals[:, 0] +
                        bary[:, 1:2] * vertex_normals[:, 1] +
                        bary[:, 2:3] * vertex_normals[:, 2]
                )
            else:
                intersection.normal[index_ray] = faceNormals[index_tri]

            if self.textureCoordinates is not None:
                textureCoordinates = ArrayUtil.convert(self.textureCoordinates, xp)
                triangles_uv = textureCoordinates[faces[index_tri]]
                bary = trimesh.triangles.points_to_barycentric(
                    vertices[faces[index_tri]],
                    locations
                )
                intersection.textureCoords[index_ray] = (
                        bary[:, 0:1] * triangles_uv[:, 0] +
                        bary[:, 1:2] * triangles_uv[:, 1] +
                        bary[:, 2:3] * triangles_uv[:, 2]
                )

            # Update upper bounds
            upper_bound[index_ray] = dist
            intersection.distance[index_ray] = dist * inv_dir_len[index_ray]
            intersection.object[index_ray] = self.id
            intersection.faceIndex[index_ray] = index_tri

            # Transform back to global
            intersection.position[index_ray] = self.meshToGlobal.apply(locations)
            intersection.normal[index_ray] = self.meshToGlobal.apply(intersection.normal[index_ray], only_linear=True)
